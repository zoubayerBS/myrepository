...
export default function MessagesPage() {
  ...
  const handleMessageClick = (message: Message) => {
    ...
    if (!message.read) {
        setMessages(prev => prev.map(m => m.id === message.id ? { ...m, read: 1 } : m));
        fetch(`/api/messages/${message.id}/read`, { method: 'PUT' });
    }
  };

  const handleMessageSent = () => {
    fetchMessages('sent'); // Refresh sent messages after sending a new one
  ...
  const handleArchiveToggle = async (messageId: string, isArchived: boolean) => {
    try {
      ...
      fetchMessages(activeTab); // Refresh current tab messages
      setSelectedMessage(null); // Deselect message after archiving
    } catch (error) {
                  ...
                  >
                    <div className="flex-1">
                      <div className='flex justify-between items-baseline'>
                        ...
                        <p className={`text-xs ${!msg.read && activeTab === 'received' ? 'text-blue-500' : 'text-gray-400'}`}>
              ...
              <div className="flex-1 flex flex-col">
                <div className="p-4 border-b flex justify-between items-center">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      {activeTab === 'archived' ? (
                        <DropdownMenuItem onClick={() => handleArchiveToggle(selectedMessage.id, false)}>
                          Désarchiver
                        </DropdownMenuItem>
                      ) : (
                        <DropdownMenuItem onClick={() => handleArchiveToggle(selectedMessage.id, true)}>
                          Archiver
                        </DropdownMenuItem>
                      )}
                    </DropdownMenuContent>
                ...
                <div className="flex items-center gap-4 mt-2 p-4">
                  <Avatar className="h-10 w-10">
                    <AvatarFallback>{selectedMessage.senderName[0].toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div>
